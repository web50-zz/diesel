...
* @package	SBIN Diesel
*/
class di_market_types extends data_interface
{
	public $title = 'Структура товаров магазина';
	/**
	...
	*/
	...
	{
		// 9* выдем только список типов по которым есть товары в каталоге с флагом on_offer - 1
		// и да запрос гавно и грязный костыль
		$sql = "select * from market_types 
			where id in (SELECT type_id from catalogue_item where type_id != 0 and on_offer = 1 group by type_id) 
			order by title asc
			";
		$res = $this->_get($sql);
		return $res;
