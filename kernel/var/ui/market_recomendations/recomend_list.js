ui.market_recomendations.recomend_list = function(config, vp){
	var formW = 700;
	var formH = 480;
	Ext.apply(this, config);
	var proxy = new Ext.data.HttpProxy({
		api: {
			read: 'di/market_recomendations/list.json',
			create: 'di/market_recomendations/set.do',
			update: 'di/market_recomendations/set.do',
			destroy: 'di/market_recomendations/unset.do'
		}
	});
	...
	var reader = new Ext.data.JsonReader({
			...
			{name: 'id', type: 'int'},
			{name: 'm_recomend_product_id', type: 'int'},
			'p_title',
			'p_type',
			'p_collection',
			'p_group'
	...
	var columns = [
		{header: this.colPid, width: 50, sortable: true, dataIndex: 'm_recomend_product_id', id: 'm_recomend_product_id'},
	...
	ui.market_recomendations.recomend_list.superclass.constructor.call(this, {
		store: store,
		columns: columns,
		tbar: [
	...
	this.Add = function(id){
		Ext.Ajax.request({
			url: 'di/market_recomendations/set.do',
			...
			params:{ m_recomend_product_id: id }
			});
	}.createDelegate(this);

};
Ext.extend(ui.market_recomendations.recomend_list, Ext.grid.GridPanel, {
	limit: 20,
	colPid: "Id товара",
