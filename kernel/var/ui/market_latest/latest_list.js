ui.market_latest.latest_list = function(config, vp){
	var formW = 700;
	var formH = 480;
	Ext.apply(this, config);
	var proxy = new Ext.data.HttpProxy({
		api: {
			read: 'di/market_latest/list.json',
			create: 'di/market_latest/set.do',
			update: 'di/market_latest/set.do',
			destroy: 'di/market_latest/unset.do'
		}
	});
	...
	var reader = new Ext.data.JsonReader({
			...
			{name: 'id', type: 'int'},
			{name: 'm_latest_product_id', type: 'int'},
			'p_title',
			'p_type',
			'p_collection',
			'p_group'
	...
	var columns = [
		{header: this.colPid, width: 50, sortable: true, dataIndex: 'm_latest_product_id', id: 'm_latest_product_id'},
	...
	ui.market_latest.latest_list.superclass.constructor.call(this, {
		store: store,
		columns: columns,
		tbar: [
	...
	this.Add = function(id){
		Ext.Ajax.request({
			...
			params:{ m_latest_product_id: id }
			});
	}.createDelegate(this);

};
Ext.extend(ui.market_latest.latest_list, Ext.grid.GridPanel, {
	limit: 20,
	colPid: "Id товара",
