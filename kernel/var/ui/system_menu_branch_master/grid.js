ui.system_menu_branch_master.grid = Ext.extend(Ext.grid.EditorGridPanel, {
	lblName: 'Имя',
	lblType: 'Тип',
	lblValue: 'Значение',
	...
	store: new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			api: {
				read: 'di/system_menu_branch_master/list.js',
				create: 'di/system_menu_branch_master/set.js',
				update: 'di/system_menu_branch_master/mset.js',
				destroy: 'di/system_menu_branch_master/unset.js'
			}
		}),
	...
	Add: function(){
		var f = new ui.system_menu_branch_master.item_form({pid:this.pid});
	...
	Import: function(){
		var f = new ui.system_menu_branch_master.item_import_form({pid:this.pid});
	...
	attachTo: function(mode){
		...
		Ext.Ajax.request({
			url: 'di/system_menu_branch_master/attach.json',
			success: function(r){
	...
	Export: function(){
		var id = this.getSelectionModel().getSelected().get('id');
		if(id>0){
			window.location.href = './di/system_menu_branch_master/export.do?_sid='+id;
		}
	},
	AddSingle:function(){
		var f = new ui.system_menu_branch_master.item_form({pid:this.pid});
	...
	Load:function(input){
		...
		if(pid>0 && id>0){
			var params = {id: id,pid:this.pid,type:type};
				Ext.Ajax.request({
					url: 'di/system_menu_branch_master/load.json',
					success: function(r){
							var d = Ext.util.JSON.decode(r.responseText);
							if (d.success == true){
								//this.store.reload();
	...
	constructor: function(config)
	{
		...
		ui.system_menu_branch_master.grid.superclass.constructor.call(this, config);
